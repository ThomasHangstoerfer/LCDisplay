        ...
        for i in range(len(self.entries)):
            if self.entries[i]["name"] != "":
                ...
                if (self.currentline == i):
                    o = o +5
                draw.text((1 + o, y_offset), self.entries[i]["name"], fill=(getTheme()["highlight_text_color"] if (self.currentline == i) else getTheme()["text_color"]), font = getTheme()["font"])
                if (self.currentline == i):
                    draw.line([(0, y_offset), (127, y_offset)], fill=getTheme()["highlight_text_color"], width=1)
                    draw.line([(0, y_offset+10), (127, y_offset+10)], fill=getTheme()["highlight_text_color"], width=1)

            y_offset += 12

        draw.text((40, 110), datetime.datetime.now().strftime('%H:%M:%S'), fill = getTheme()["highlight_text_color"], font = getTheme()["clockfont"])
